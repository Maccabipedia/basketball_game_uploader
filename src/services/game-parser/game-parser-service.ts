...
export class GameParserService extends BaseService implements IGameParserService {
    ...
    parsePlayer(player: IPlayer) {
        ...
        const fields = [
            ...
            `נק=${pointCount || ''}`,
            `זריקות עונשין=${this.services.util.isValidNumber(player.freeThrowsAttempts) ? player.freeThrowsAttempts : ''}`,
            `קליעות עונשין=${this.services.util.isValidNumber(player.freeThrowsScored) ? player.freeThrowsScored : ''}`,
            `זריקות שתי נק=${this.services.util.isValidNumber(player.fieldThrowsAttempts) ? player.fieldThrowsAttempts : ''}`,
            `קליעות שתי נק=${this.services.util.isValidNumber(player.fieldThrowsScored) ? player.fieldThrowsScored : ''}`,
            `זריקות שלוש נק=${this.services.util.isValidNumber(player.threeThrowsAttempts) ? player.threeThrowsAttempts : ''}`,
            `קליעות שלוש נק=${this.services.util.isValidNumber(player.threeThrowsScored) ? player.threeThrowsScored : ''}`,
            `ריבאונד הגנה=${this.services.util.isValidNumber(player.defensiveRebounds) ? player.defensiveRebounds : ''}`,
            `ריבאונד התקפה=${this.services.util.isValidNumber(player.offensiveRebounds) ? player.offensiveRebounds : ''}`,
            `פאולים=${this.services.util.isValidNumber(player.personalTotalFouls) ? player.personalTotalFouls : ''}`,
            `חטיפות=${this.services.util.isValidNumber(player.steals) ? player.steals : ''}`,
            `איבודים=${this.services.util.isValidNumber(player.turnovers) ? player.turnovers : ''}`,
            `אסיסטים=${this.services.util.isValidNumber(player.assists) ? player.assists : ''}`,
            `בלוקים=${this.services.util.isValidNumber(player.blocks) ? player.blocks : ''}`,
        ]
